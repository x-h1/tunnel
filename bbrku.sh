...
Install_BBR(){
echo -e "[ \e[0;32mINFO\e[0m ] Install TCP BBR"
if [ -n "$(lsmod | grep bbr)" ];then
echo -e "[ \e[0;32mINFO\e[0m ] Succesfully Installed TCP BBR"
sleep 1 
return 1
fi
echo -e "[ \e[0;32mINFO\e[0m ] Mulai menginstall TCP_BBR"
sleep 1
modprobe tcp_bbr
...
if [ -n "$(sysctl net.ipv4.tcp_available_congestion_control | grep bbr)" ] && [ -n "$(sysctl net.ipv4.tcp_congestion_control | grep bbr)" ] && [ -n "$(lsmod | grep "tcp_bbr")" ];then
	echo -e "[ \e[0;32mINFO\e[0m ] TCP_BBR Install Success"
	sleep 1
else
	echo -e "[ \e[0;32mINFO\e[0m ] Gagal menginstall TCP_BBR"
	sleep 1
fi
echo -e " "
}
...
Optimize_Parameters(){
echo -e "[ \e[0;32mINFO\e[0m ] Optimasi Parameters Kernel"
sleep 1
Check_And_Add_Line "/etc/security/limits.conf" "* soft nofile 1000000"
Check_And_Add_Line "/etc/security/limits.conf" "* hard nofile 1000000"
...
echo -e "[ \e[0;32mINFO\e[0m ] Succesfully Optimize Parameters Kernel"
}
Install_BBR
Optimize_Parameters
rm -f /root/bbrku.sh
